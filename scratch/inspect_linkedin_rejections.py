...
async def main():
    ...
    async with pool.acquire() as conn:
        ...
        for rj in plan.rejected_jobs:
            reasons.setdefault(rj.reason, []).append((rj.job.title, rj.job.company, rj.job.min_experience))

        for reason, jlist in reasons.items():
            print(f"\nRejection Reason: {reason} ({len(jlist)} jobs)")
            for title, comp, exp in jlist:
                print(f"  - {title} @ {comp} | min_exp: {exp}")

        print("\nEligible LinkedIn Jobs:")
        for ej in plan.eligible_jobs:
            print(f"  * [Score {ej.score:.1f}] {ej.job.title} @ {ej.job.company} (ID: {ej.job.job_id})")
