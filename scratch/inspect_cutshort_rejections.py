...
async def main():
    ...
    async with pool.acquire() as conn:
        ...
        print(f"Loaded {len(jobs)} Cutshort jobs.")
        ...
        for rj in plan.rejected_jobs:
            reasons.setdefault(rj.reason, []).append((rj.job.title, rj.job.company, rj.job.experience_text))

        for reason, jlist in reasons.items():
            print(f"\nRejection Reason: {reason} ({len(jlist)} jobs)")
            for title, comp, exp in jlist[:8]:
                print(f"  - {title} @ {comp} (exp: {exp})")
